...
function ActionBar({ post, onLike, onRepost }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', marginTop: 8, marginLeft: -10 }}>
      ...
      <button style={{ ...btnBase, color: 'var(--text-muted)', marginLeft: 'auto' }}
        onMouseEnter={e => { e.currentTarget.style.background='rgba(99,102,241,0.08)'; e.currentTarget.style.color='#6366f1'; }}
        onMouseLeave={e => { e.currentTarget.style.background='none'; e.currentTarget.style.color='var(--text-muted)'; }}>
...
function Post({ post, onLike, onRepost }) {
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {/* Embedded screener card */}
        {post.screener && (
          <div style={{ border: '1px solid var(--border2)', borderRadius: 12, padding: '12px 14px', marginBottom: 10, transition: 'background 0.1s' }}
        ...
        <ActionBar post={post} onLike={onLike} onRepost={onRepost} />
      </div>
    </div>
  );
}
...
export default function FeedTab() {
  ...
  const [activeDir,  setActiveDir]  = useState(null);   // 'Long' | 'Short' | null
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', fontFamily: 'var(--font)' }}>
      ...
      <div style={{ borderRight: '1px solid var(--border)' }}>
        ...
        {/* Posts */}
        {posts.map(post => (
          <Post key={post.id} post={post} onLike={handleLike} onRepost={handleRepost} />
        ))}
      </div>
