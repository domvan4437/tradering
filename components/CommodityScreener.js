...
const SECTION_TABS = {
  ...
  compete:     ['Home','H2H','Group Contests','History'],
                  ...
                  {hasDropdown && isHovered && (
                    <div style={{ position:'absolute', top:'100%', left:'50%', transform:'translateX(-50%)', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, boxShadow:'0 8px 24px rgba(0,0,0,0.12)', zIndex:99999, minWidth:190, padding:'6px 0' }}>
                      {sec === 'tools2' ? TOOLS_LIST.map(tool => (
                        <button
                          key={tool.key}
                          onClick={() => { setSection(sec); setTab(tool.key); setHoveredSection(null); }}
                          style={{ display:'flex', alignItems:'center', gap:8, width:'100%', padding:'9px 16px', background:'none', border:'none', cursor:'pointer', fontFamily:'var(--font)', fontSize:13, color:'var(--text-muted)', textAlign:'left' }}
...
function TradeCalcTab() {
  const CONTRACT_SIZES = [
    { name:'Gold (GC)',        size:'100',    unit:'oz',    exchange:'COMEX', tick:'$0.10 = $10'      },
    { name:'Silver (SI)',      size:'5000',   unit:'oz',    exchange:'COMEX', tick:'$0.005 = $25'     },
    { name:'Crude Oil (CL)',   size:'1000',   unit:'bbl',   exchange:'NYMEX', tick:'$0.01 = $10'      },
    { name:'Natural Gas (NG)', size:'10000',  unit:'MMBtu', exchange:'NYMEX', tick:'$0.001 = $10'     },
    { name:'Corn (ZC)',        size:'5000',   unit:'bu',    exchange:'CBOT',  tick:'$0.0025 = $12.50' },
    { name:'Wheat (ZW)',       size:'5000',   unit:'bu',    exchange:'CBOT',  tick:'$0.0025 = $12.50' },
    { name:'Soybeans (ZS)',    size:'5000',   unit:'bu',    exchange:'CBOT',  tick:'$0.0025 = $12.50' },
    { name:'Coffee (KC)',      size:'37500',  unit:'lbs',   exchange:'ICE',   tick:'$0.0005 = $18.75' },
    { name:'Sugar (SB)',       size:'112000', unit:'lbs',   exchange:'ICE',   tick:'$0.0001 = $11.20' },
    { name:'Cotton (CT)',      size:'50000',  unit:'lbs',   exchange:'ICE',   tick:'$0.0001 = $5'     },
    { name:'Live Cattle (LE)', size:'40000',  unit:'lbs',   exchange:'CME',   tick:'$0.00025 = $10'   },
    { name:'E-mini S&P (ES)',  size:'50',     unit:'pts',   exchange:'CME',   tick:'0.25 = $12.50'    },
    { name:'E-mini NQ (NQ)',   size:'20',     unit:'pts',   exchange:'CME',   tick:'0.25 = $5'        },
    { name:'Euro FX (6E)',     size:'125000', unit:'EUR',   exchange:'CME',   tick:'$0.00005 = $6.25' },
  ];

  const [form, setForm] = useState({ account:'50000', risk:'1', entry:'', stop:'', contractSize:'100', direction:'BUY', symbol:'' });
  const [savedCalcs, setSavedCalcs] = useState(() => { try { return JSON.parse(localStorage.getItem('tr_saved_calcs')||'[]'); } catch { return []; } });
  ...
  const calc = useMemo(() => {
    ...
    const csize   = parseFloat(form.contractSize);
    if (!account || !riskPct || !entry || !stop || !csize || entry === stop) return null;
    const isBuy           = form.direction === 'BUY';
    const riskPerTrade    = account * (riskPct / 100);
    const stopDistance    = Math.abs(entry - stop);
    const riskPerContract = stopDistance * csize;
    ...
    const p1              = contracts * stopDistance * csize;
    const t = (mult) => isBuy ? entry + stopDistance * mult : entry - stopDistance * mult;
    const breakeven       = isBuy ? entry + (riskPerContract / csize) : entry - (riskPerContract / csize);
    return {
      contracts, actualRiskPct: actualRiskPct.toFixed(2),
      riskPerTrade: riskPerTrade.toFixed(2), actualRisk: actualRisk.toFixed(2),
      riskPerContract: riskPerContract.toFixed(2), stopDistance: stopDistance.toFixed(4),
      t1: t(1).toFixed(2), t2: t(2).toFixed(2), t3: t(3).toFixed(2), t5: t(5).toFixed(2),
      p1: p1.toFixed(2), p2: (p1*2).toFixed(2), p3: (p1*3).toFixed(2), p5: (p1*5).toFixed(2),
      breakeven: breakeven.toFixed(4),
      streak5: (actualRisk * 5).toFixed(2), streak10: (actualRisk * 10).toFixed(2),
      accountAfter2R: (account + p1*2).toFixed(0), accountAfter3R: (account + p1*3).toFixed(0),
    };
  }, [form]);
  ...
  const saveCalc = () => {
    if (!calc) return;
    const entry = {
      id: Date.now(), label: form.symbol || (form.entry + ' / ' + form.stop),
      direction: form.direction, contracts: calc.contracts,
      risk: calc.actualRisk, entry: form.entry, stop: form.stop,
      t2: calc.t2, t3: calc.t3, date: new Date().toLocaleDateString(),
    };
    const updated = [entry, ...savedCalcs].slice(0, 20);
    setSavedCalcs(updated);
    try { localStorage.setItem('tr_saved_calcs', JSON.stringify(updated)); } catch {}
  };

  const deleteSaved = (id) => {
    const updated = savedCalcs.filter(sc => sc.id !== id);
    setSavedCalcs(updated);
    try { localStorage.setItem('tr_saved_calcs', JSON.stringify(updated)); } catch {}
  };

  const inp = { width:'100%', background:'var(--surface2)', border:'1px solid var(--border)', padding:'9px 12px', fontSize:14, color:'var(--text)', outline:'none', fontFamily:'var(--font)', boxSizing:'border-box', borderRadius:8 };
  const riskLevel = parseFloat(form.risk);
  const riskColor = riskLevel <= 1 ? 'var(--green)' : riskLevel <= 2 ? '#f59e0b' : 'var(--red)';
  const riskLabel = riskLevel <= 1 ? 'Conservative' : riskLevel <= 2 ? 'Moderate' : riskLevel <= 3 ? 'Aggressive' : 'Danger Zone';
    ...
    <div style={{ fontFamily:'var(--font)' }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <div>
          ...
          <h2 style={{ fontSize:24, fontWeight:700, color:'var(--text)', margin:'0 0 4px' }}>Trade Calculator</h2>
          <p style={{ fontSize:12, color:'var(--text-muted)', margin:0 }}>Position sizing · R-multiple targets · Risk management</p>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          <button onClick={() => setShowSaved(s => !s)} style={{ padding:'7px 14px', borderRadius:8, border:'1px solid var(--border)', background: showSaved?'#4f46e5':'var(--surface2)', color: showSaved?'#fff':'var(--text-muted)', fontFamily:'var(--font)', fontSize:12, fontWeight:600, cursor:'pointer' }}>
            Saved {savedCalcs.length > 0 && '(' + savedCalcs.length + ')'}
          </button>
          {calc && <button onClick={saveCalc} style={{ padding:'7px 14px', borderRadius:8, border:'none', background:'#4f46e5', color:'#fff', fontFamily:'var(--font)', fontSize:12, fontWeight:700, cursor:'pointer' }}>+ Save</button>}
        </div>
      </div>

      {showSaved && (
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'14px 18px', marginBottom:16 }}>
          <div style={{ fontSize:13, fontWeight:700, color:'var(--text)', marginBottom:10 }}>Saved Calculations</div>
          {savedCalcs.length === 0 ? (
            <div style={{ fontSize:12, color:'var(--text-muted)', textAlign:'center', padding:'16px 0' }}>No saved calculations yet.</div>
          ) : (
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {savedCalcs.map(sc => (
                <div key={sc.id} style={{ display:'grid', gridTemplateColumns:'1fr 60px 80px 80px 80px 32px', gap:10, padding:'8px 12px', background:'var(--surface2)', borderRadius:8, alignItems:'center' }}>
                  <div><div style={{ fontSize:12, fontWeight:600, color:'var(--text)' }}>{sc.label}</div><div style={{ fontSize:10, color:'var(--text-muted)' }}>{sc.date}</div></div>
                  <div style={{ fontSize:11, color: sc.direction==='BUY'?'var(--green)':'var(--red)', fontWeight:700 }}>{sc.direction}</div>
                  <div style={{ fontSize:12, fontFamily:'var(--font-mono)', color:'#4f46e5', fontWeight:700 }}>{sc.contracts} cts</div>
                  <div style={{ fontSize:12, fontFamily:'var(--font-mono)', color:'var(--red)' }}>{'- $' + sc.risk}</div>
                  <div style={{ fontSize:11, color:'var(--text-muted)' }}>2R: {sc.t2}</div>
                  <button onClick={() => deleteSaved(sc.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', fontSize:16 }} onMouseEnter={e=>e.currentTarget.style.color='var(--red)'} onMouseLeave={e=>e.currentTarget.style.color='var(--text-muted)'}>x</button>
                </div>
              ))}
            </div>
          )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, alignItems:'start' }}>
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14, padding:'20px 22px' }}>
          <div style={{ fontSize:12, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:16 }}>Inputs</div>

          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
            <div>
              <div style={{ fontSize:11, fontWeight:600, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:5 }}>Symbol (optional)</div>
              <input value={form.symbol} onChange={e=>set('symbol',e.target.value)} placeholder="e.g. Gold, ES, CL" style={inp} />
            </div>
            <div>
              ...
              <div style={{ display:'flex', gap:6 }}>
                {['BUY','SELL'].map(d=>(
                  <button key={d} onClick={()=>set('direction',d)} style={{ flex:1, padding:'9px', borderRadius:8, border:'none', fontFamily:'var(--font)', fontSize:13, fontWeight:700, cursor:'pointer', background: form.direction===d?(d==='BUY'?'var(--green)':'var(--red)'):'var(--surface2)', color: form.direction===d?'#fff':'var(--text-muted)' }}>{d}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
            <div>
              <div style={{ fontSize:11, fontWeight:600, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:5 }}>Account Size ($)</div>
              <div style={{ position:'relative' }}>
                <span style={{ position:'absolute', left:10, top:'50%', transform:'translateY(-50%)', fontSize:13, color:'var(--text-muted)' }}>$</span>
                <input value={form.account} onChange={e=>set('account',e.target.value)} type="number" style={{ ...inp, paddingLeft:22 }} onFocus={e=>e.target.style.borderColor='#4f46e5'} onBlur={e=>e.target.style.borderColor='var(--border)'} />
              </div>
            </div>
            <div>
              <div style={{ fontSize:11, fontWeight:600, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:5 }}>Risk Per Trade</div>
              <div style={{ position:'relative' }}>
                <input value={form.risk} onChange={e=>set('risk',e.target.value)} type="number" step="0.1" style={{ ...inp, paddingRight:28 }} onFocus={e=>e.target.style.borderColor='#4f46e5'} onBlur={e=>e.target.style.borderColor='var(--border)'} />
                <span style={{ position:'absolute', right:10, top:'50%', transform:'translateY(-50%)', fontSize:13, color:'var(--text-muted)' }}>%</span>
              </div>
              {form.risk && <div style={{ fontSize:10, fontWeight:600, color:riskColor, marginTop:3 }}>{riskLabel}</div>}
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:14 }}>
            <div>
              ...
              <input value={form.entry} onChange={e=>set('entry',e.target.value)} placeholder="2350.00" type="number" style={inp} onFocus={e=>e.target.style.borderColor='#4f46e5'} onBlur={e=>e.target.style.borderColor='var(--border)'} />
            </div>
            <div>
              <div style={{ fontSize:11, fontWeight:600, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:5 }}>Stop Loss</div>
              <input value={form.stop} onChange={e=>set('stop',e.target.value)} placeholder="2300.00" type="number" style={inp} onFocus={e=>e.target.style.borderColor='#4f46e5'} onBlur={e=>e.target.style.borderColor='var(--border)'} />
            </div>
          </div>

          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:11, fontWeight:600, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:5 }}>Contract Size</div>
            <input value={form.contractSize} onChange={e=>set('contractSize',e.target.value)} type="number" style={inp} onFocus={e=>e.target.style.borderColor='#4f46e5'} onBlur={e=>e.target.style.borderColor='var(--border)'} />
          </div>

          <div>
            <div style={{ fontSize:11, fontWeight:600, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:6 }}>Quick Select Contract</div>
            <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
              {CONTRACT_SIZES.slice(0,10).map(c=>(
                <button key={c.name} onClick={()=>set('contractSize',c.size)} style={{ padding:'4px 9px', borderRadius:20, border:'1px solid '+(form.contractSize===c.size?'#4f46e5':'var(--border)'), background: form.contractSize===c.size?'rgba(79,70,229,0.12)':'transparent', color: form.contractSize===c.size?'#4f46e5':'var(--text-muted)', fontFamily:'var(--font)', fontSize:11, fontWeight:600, cursor:'pointer' }}>
                  {c.name.split(' ')[0]}
                </button>
              ))}
            </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {!calc ? (
            <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14, padding:'48px 24px', textAlign:'center' }}>
              ...
              <div style={{ fontSize:15, fontWeight:600, color:'var(--text)', marginBottom:6 }}>Enter entry and stop to calculate</div>
              <div style={{ fontSize:12, color:'var(--text-muted)' }}>Results update live as you type</div>
            </div>
          ) : (
            <>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                {[
                  { label:'Contracts',      value: calc.contracts,            color:'#4f46e5',           sub: calc.contracts===0?'Too small':'Max position size', big:true },
                  { label:'Max Risk $',     value: '$' + calc.riskPerTrade,   color:'var(--text)',        sub: form.risk + '% of account' },
                  { label:'Actual Risk $',  value: '$' + calc.actualRisk,     color:'var(--red)',         sub: calc.actualRiskPct + '% of account' },
                  { label:'Risk/Contract',  value: '$' + calc.riskPerContract,color:'var(--text-muted)',  sub: 'Stop dist x contract size' },
                ].map(s=>(
                  <div key={s.label} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'14px 16px' }}>
                    <div style={{ fontSize:10, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:6 }}>{s.label}</div>
                    <div style={{ fontFamily:'var(--font-mono)', fontSize:s.big?26:20, fontWeight:800, color:s.color, lineHeight:1 }}>{s.value}</div>
                    <div style={{ fontSize:10, color:'var(--text-muted)', marginTop:5 }}>{s.sub}</div>
                  </div>
                ))}
              </div>

              <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'14px 16px' }}>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:12 }}>R-Multiple Targets</div>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
                  {[
                    { r:'1R', price:calc.t1, profit:calc.p1 },
                    { r:'2R', price:calc.t2, profit:calc.p2 },
                    { r:'3R', price:calc.t3, profit:calc.p3 },
                    { r:'5R', price:calc.t5, profit:calc.p5 },
                  ].map(t=>(
                    <div key={t.r} style={{ background:'var(--surface2)', borderRadius:10, padding:'10px', border:'1px solid var(--border)', textAlign:'center' }}>
                      <div style={{ fontSize:10, fontWeight:700, color: isBuy?'var(--green)':'var(--red)', marginBottom:5 }}>{t.r}</div>
                      <div style={{ fontFamily:'var(--font-mono)', fontSize:13, fontWeight:700, color:'var(--text)', marginBottom:3 }}>{t.price}</div>
                      <div style={{ fontFamily:'var(--font-mono)', fontSize:11, color: isBuy?'var(--green)':'var(--red)', fontWeight:700 }}>{'+ $' + t.profit}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'14px 16px' }}>
                <div style={{ fontSize:11, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:12 }}>Risk Insights</div>
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {[
                    { label:'Break-even price',        value: calc.breakeven,                              color:'var(--text)'   },
                    { label:'5 consecutive losses',    value: '- $' + calc.streak5,                       color:'var(--red)'    },
                    { label:'10 consecutive losses',   value: '- $' + calc.streak10,                      color:'var(--red)'    },
                    { label:'Account after 2R win',    value: '$' + parseInt(calc.accountAfter2R).toLocaleString(), color:'var(--green)'  },
                    { label:'Account after 3R win',    value: '$' + parseInt(calc.accountAfter3R).toLocaleString(), color:'var(--green)'  },
                  ].map(row=>(
                    <div key={row.label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 10px', background:'var(--surface2)', borderRadius:8 }}>
                      <span style={{ fontSize:12, color:'var(--text-muted)' }}>{row.label}</span>
                      <span style={{ fontFamily:'var(--font-mono)', fontSize:12, fontWeight:700, color:row.color }}>{row.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {calc.contracts === 0 && (
                <div style={{ padding:'12px 16px', background:'rgba(255,50,80,0.08)', border:'1px solid rgba(255,50,80,0.25)', borderRadius:10 }}>
                  <div style={{ fontSize:13, color:'var(--red)', fontWeight:600, marginBottom:3 }}>Position size is 0 contracts</div>
                  <div style={{ fontSize:12, color:'var(--text-muted)' }}>Risk amount is less than 1 contract cost. Increase account size, raise risk %, or widen stop.</div>
                </div>
              )}
            </>
          )}
      ...
      <div style={{ marginTop:20 }}>
        <div style={{ fontSize:13, fontWeight:700, color:'var(--text)', marginBottom:10 }}>Contract Size Reference</div>
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, overflow:'hidden' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 90px 80px 80px 130px', padding:'8px 16px', background:'var(--surface2)', borderBottom:'1px solid var(--border)' }}>
            {['Contract','Size','Unit','Exchange','Tick Value'].map(h=>(
              <div key={h} style={{ fontSize:10, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.08em' }}>{h}</div>
            ))}
          </div>
          {CONTRACT_SIZES.map((c,i)=>(
            <div key={c.name} onClick={()=>set('contractSize',c.size)}
              style={{ display:'grid', gridTemplateColumns:'1fr 90px 80px 80px 130px', padding:'9px 16px', borderBottom: i<CONTRACT_SIZES.length-1?'1px solid var(--border)':'none', cursor:'pointer', transition:'background 0.1s', borderLeft: form.contractSize===c.size?'3px solid #4f46e5':'3px solid transparent' }}
              onMouseEnter={e=>e.currentTarget.style.background='var(--surface2)'}
              onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
              <div style={{ fontSize:13, color: form.contractSize===c.size?'#4f46e5':'var(--text)', fontWeight: form.contractSize===c.size?600:400 }}>{c.name}</div>
              <div style={{ fontFamily:'var(--font-mono)', fontSize:12, color:'var(--text)' }}>{parseInt(c.size).toLocaleString()}</div>
              <div style={{ fontSize:12, color:'var(--text-muted)' }}>{c.unit}</div>
              <div style={{ fontSize:12, color:'var(--text-muted)' }}>{c.exchange}</div>
              <div style={{ fontSize:11, color:'var(--text-muted)' }}>{c.tick}</div>
            </div>
          ))}
        </div>
      </div>
